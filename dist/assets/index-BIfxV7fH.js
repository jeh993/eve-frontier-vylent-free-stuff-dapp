import{C as e,S as t,_ as n,a as r,b as i,c as a,f as o,h as s,i as c,l,m as u,n as d,o as f,p,r as m,t as h,v as g,x as _}from"./world-0VWNJrQF.js";var v=e((async()=>{_(),u(),l(),i();var e=new URLSearchParams(window.location.search).get(`storage`),f=document.getElementById(`trade-terminal-output`),p=document.getElementById(`market-output`),v=document.getElementById(`credits`),y=`buy`;document.getElementById(`buy-tab`).addEventListener(`click`,async()=>{y=`buy`,T(C,await A()),N()}),document.getElementById(`sell-tab`).addEventListener(`click`,async()=>{y=`sell`,T(C,await A()),N()});var b=await r(e),x=`/eve-frontier-vylent-free-stuff-dapp/dist/types-index.json`,S=null,C=await Promise.all(b.map(async e=>{let t=Number(e.fields.item_id);return console.log(`ITEM ID`,t,await j(t)),{itemId:t,name:await j(t),buyPrice:Number(e.fields.buy_from_store),sellPrice:Number(e.fields.sell_to_store)}}));N(),v.innerHTML=`CREDITS: <a href="#" id="connect-credits">[CONNECT WALLET]</a>`,T(C,await k()),document.getElementById(`connect-credits`).addEventListener(`click`,async e=>{e.preventDefault(),await O(),T(C,await A())});function w(e){f.style.display=`block`,f.innerHTML+=`${e}<br>`}function T(e,t){p.innerHTML=`
